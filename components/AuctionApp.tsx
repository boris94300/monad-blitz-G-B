...
function RoomPage({config,code,role}:{config:Config|null;code:string;role:'host'|'join'|'scene'}){
  ...
  useEffect(()=>{if(!live.stream)setCameraReady(false);},[live.stream]);
  ...
  async function scan(rehearsal=false){
    ...
    if(rehearsal){
      ...
      const v=videoRef.current;if(!v?.videoWidth||!live.stream)throw new Error('Activez la caméra et cadrez quelques objets.');
      const canvas=document.createElement('canvas');canvas.width=Math.min(v.videoWidth,960);canvas.height=canvas.width*v.videoHeight/v.videoWidth;canvas.getContext('2d')!.drawImage(v,0,0,canvas.width,canvas.height);image=canvas.toDataURL('image/jpeg',.8);
  ...
  return <div className={`room-page ${scene?'scene-mode':''}`}>
    ...
    <div className="room-grid"><section className="broadcast-column">
      <div className="video-shell"><div className="video-top"><Pill className={room?.live?'live-pill':'offline-pill'}><span className="dot"/>{room?.live?'EN DIRECT':'EN ATTENTE'}</Pill><span className="video-source">{host?'CAMÉRA VENDEUR':live.rtcConnected?'VIDÉO EN DIRECT':frameFresh?'DIRECT · DÉBIT RÉDUIT':'LE SPECTACLE VA COMMENCER'}</span></div>
      {host&&live.stream?<Video stream={live.stream} muted onReady={readyVideo}/>:!host&&live.remote&&live.rtcConnected?<Video stream={live.remote} muted={muted}/>:!host&&frameFresh?<img className="live-image" src={live.frame} alt="Images en direct du vendeur"/>:<div className="video-empty"><div className="camera-orbit"><Camera size={44} strokeWidth={1}/></div><h2>{host?'Votre pièce. Notre prochain scandale.':room?.live?'Connexion au live…':'Le vendeur prépare son bazar.'}</h2><p>{host?'Activez la caméra pour ouvrir la vente la moins sérieuse du siècle.':'La vidéo apparaîtra ici dès que le vendeur démarre.'}</p>{host&&<button className="button lime" disabled={!!busy||!connected} onClick={()=>act('Ouverture de la caméra…',()=>live.start(mic))}><Camera size={18}/> Activer la caméra</button>}</div>}
