...
function RoomPage({config,code,role}:{config:Config|null;code:string;role:'host'|'join'|'scene'}){
  ...
  useEffect(()=>{
    if(!host||!socket)return;let cancelled=false;let timer:ReturnType<typeof setTimeout>;let pass=0;
    async function tick(){
      const state=trackingSource.current;
      if(state.room?.live&&!state.busy){
        try{
          ...
          if(canvas.width&&canvas.height&&(v?.videoWidth||state.live.frame)){
            const {detect}=await import('@/lib/detector');const candidates=selectCandidates(await detect(canvas,++pass%3===0),state.includePeople);
    ...
    timer=setTimeout(tick,1500);return()=>{cancelled=true;clearTimeout(timer);};
  },[host,socket]);
  ...
  return <div className={`room-page ${scene?'scene-mode':''}`}>
    ...
    <div className="room-grid"><section className="broadcast-column">
      ...
      {host?<button className="button primary full buy-button" disabled={!!busy||!connected||lot.status!=='preview'} onClick={()=>act('Mise en vente…',startLot)}>{lot.status==='active'?<><Radio size={19}/> Les enchères sont ouvertes</>:<><Zap size={19}/> {lots.length>1?`Lancer les ${lots.length} enchères`:"Lancer l’enchère"}</>}</button>:scene?<div className="scene-instruction">Scannez le QR code pour acheter. ↓</div>:<>{room?.mode==='demo'&&<label className="nickname-label">Votre nom de collectionneur<input value={nickname} maxLength={24} onChange={e=>setNickname(e.target.value)}/></label>}<button className="button primary full buy-button" disabled={!!busy||!connected||lot.status!=='active'} onClick={()=>act(room?.mode==='demo'?'Adjugé ?':'Confirmez dans votre wallet…',buy)}>{lot.status==='preview'?'L’enchère va commencer':<>{room?.mode==='demo'?'Je craque · achat simulé':'Je craque · acheter'} <ArrowUpRight size={20}/></>}</button></>}
