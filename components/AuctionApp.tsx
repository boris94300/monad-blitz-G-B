    ...
    <main>
      <section className="hero">
        <div className="hero-copy"><div className="eyebrow"><span className="tiny-star">✳</span> MAISON DE VENTE · OBJETS EXTRAORDINAIREMENT ORDINAIRES</div>
          ...
          <p className="hero-description">Votre pièce devient une salle de vente. Cinq objets, repérés par IA. Les prix descendent en direct. <strong>Le premier qui craque gagne.</strong></p>
...
function RoomPage({config,code,role}:{config:Config|null;code:string;role:'host'|'join'|'scene'}){
  ...
  const trackingSource=useRef({live,room,busy,includePeople});trackingSource.current={live,room,busy,includePeople};
  useEffect(()=>{
      ...
      if(state.room?.live){
        try{
          ...
          if(canvas.width&&canvas.height&&(v?.videoWidth||state.live.frame)){
            const {detect}=await import('@/lib/detector');const focus=++pass%2===0?(state.room.lots||[]).filter(l=>l.source!=='manual'&&l.source!=='rehearsal').map(l=>memory.get(l.detectionLabel||l.label)?.candidate.bbox||l.bbox):[];const detected=selectCandidates(await detect(canvas,false,focus),state.includePeople);const candidates=stabilize(memory,detected,Date.now(),state.includePeople);
  ...
  async function scan(rehearsal=false){
    ...
    if(rehearsal){
      ...
      if(!remoteVision){setBusy('L’IA examine la pièce…');const {detect}=await import('@/lib/detector');candidates=await detect(canvas,true);}
    }
    ...
    setSelectedId('');setNotice(`${data.count} catégorie${data.count>1?'s':''} sélectionnée${data.count>1?'s':''}. Les prix descendront ensemble au lancement.`);
  }
  ...
  return <div className={`room-page ${scene?'scene-mode':host?'host-view':'buyer-view'}`}>
    ...
    <main className="room-main"><div className="room-title"><div><div className="eyebrow">{host?'LA RÉGIE DU CHAOS':scene?'LA SALLE DES VENTES':'EN DIRECT · À VOUS DE JOUER'}</div><h1>{room?.name||'La salle se prépare…'}</h1></div><Pill><Users size={14}/>{room?.viewers||0} dans la salle</Pill></div>
    ...
    <div className="room-grid"><section className="broadcast-column">
      ...
      {host?<div className="host-controls"><div className="control-heading"><h3><ScanLine size={18}/> L’expert entre en scène.</h3><span className="tiny-label">{config?.vision&&remoteVision?'VISION DISTANTE':'IA LOCALE · SANS CLÉ API'}</span></div><div className="control-options"><label className="check-label"><input type="checkbox" checked={includePeople} onChange={e=>setIncludePeople(e.target.checked)}/> Inclure les personnages consentants</label><label className="check-label"><input type="checkbox" checked={mic} disabled={!!live.stream} onChange={e=>setMic(e.target.checked)}/> Micro du vendeur</label><label className="duration">Durée <select value={duration} onChange={e=>setDuration(Number(e.target.value))}><option value={30}>30 s</option><option value={60}>60 s</option><option value={90}>90 s</option></select></label>{config?.vision&&<label className="check-label"><input type="checkbox" checked={remoteVision} onChange={e=>setRemoteVision(e.target.checked)}/> Expertise IA distante</label>}</div><div className="control-buttons"><button className="button small-button" onClick={()=>setPhonePanel(!phonePanel)}><Camera size={16}/> Filmer avec mon téléphone</button><button className="button primary" disabled={!!busy||!room?.live||!(cameraReady||frameFresh)||!connected||hasActive} onClick={()=>act('Sélection de la prochaine victime…',()=>scan())}><Shuffle size={18}/> Sélectionner jusqu’à 5 objets</button><button className="button small-button" disabled={!!busy||!connected||!room?.live||!(cameraReady||frameFresh)||hasActive||lots.filter(l=>l.status==='preview').length>=5} onClick={()=>act('Capture du détail…',captureDetail)}><ScanLine size={16}/> Cadrer un petit objet</button>{room?.videoSource==='phone'&&<button className="button small-button" onClick={()=>act('Arrêt du téléphone caméra…',async()=>{await api(`/api/rooms/${code}/close`,{},token);})}><VideoOff size={16}/> Couper le téléphone</button>}{live.stream&&<button className="button small-button" onClick={live.stop}><VideoOff size={16}/> Couper le live</button>}{room?.mode==='demo'&&<button className="button text-button small-button" disabled={!!busy||!connected||hasActive} onClick={()=>act('Préparation du décor…',()=>scan(true))}>Essayer avec un décor fictif</button>}</div><p className="muted small">{includePeople?'Les personnes deviennent des cartes de personnages fictifs, avec leur accord. Aucun humain n’est vendu.':'Priorité aux petits objets · une seule cible par catégorie · jusqu’à 5 lots simultanés.'}</p>{trackingError&&<p className="warning">{trackingError}</p>}<p className="recognition-note">La reconnaissance locale couvre 80 catégories : mouchoirs, écouteurs et miettes peuvent ne pas être reconnus. Rapprochez la caméra ou utilisez « Cadrer un petit objet » pour les ajouter.</p></div>:<div className="auctioneer"><span className="auctioneer-avatar">B.</span><div><span className="tiny-label">BALTHAZAR · EXPERT AUTOPROCLAMÉ</span><p>« {lot?.status==='sold'?'Adjugé. Une décision discutable, mais une décision historique.':lot?.status==='active'?auctioneer(progress):'Mes qualifications ? Une confiance en moi absolument déraisonnable.'} »</p></div></div>}
      ...
      {host?<button className="button primary full buy-button" disabled={!!busy||!connected||lot.status!=='preview'} onClick={()=>act('Mise en vente…',startLot)}>{lot.status==='active'?<><Radio size={19}/> Les enchères sont ouvertes</>:<><Zap size={19}/> {lots.length>1?`Lancer les ${lots.length} enchères`:"Lancer l’enchère"}</>}</button>:scene?<div className="scene-instruction">Scannez le QR code pour acheter. ↓</div>:<>{room?.mode==='demo'&&<label className="nickname-label">Votre nom de collectionneur<input value={nickname} maxLength={24} onChange={e=>setNickname(e.target.value)}/></label>}<button className="button primary full buy-button" disabled={!!busy||!connected||lot.status!=='active'} onClick={()=>act(room?.mode==='demo'?'Adjugé ?':'Confirmez dans votre wallet…',buy)}>{lot.status==='preview'?'L’enchère va commencer':<>{room?.mode==='demo'?'Acheter · simulation':'Acheter'} <ArrowUpRight size={20}/></>}</button></>}
